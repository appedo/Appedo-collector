...
public class JStackDataCollectorResource extends Resource {
	
	public JStackDataCollectorResource(Context context, Request request, Response response) {
		...
		try {
			...
			if (rawData.startsWith("{") && rawData.endsWith("}")) {
				...
				if (joRawData.containsKey("counterMessage")) {
					...
					bQueued = collectorManager.collectJStackData(joCounterData);
					if( bQueued ) {
