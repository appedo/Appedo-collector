 ...
 *
 */
public class MSSQLProcedureDBI {
	
	private String strMSSQLSlowProceduresQuery = null;
...
//	private PreparedStatement psDotNetProfiler = null;
	...
	private Statement stmtMSSQLSlowProcedure = null;
...
//	private String strUID = "";
	...
	public void initializeInsertBatch(Connection con) throws Exception {
		...
		try{
			strMSSQLSlowProceduresQuery = "INSERT INTO mssql_slowprocedure_@KEY@ (uid,appedo_received_on, procedure, calls, duration_ms) VALUES ";
			
			stmtMSSQLSlowProcedure = con.createStatement();
			
			pstmtPartitionTables = con.prepareStatement("SELECT create_asd_daily_partition_tables(?, ?, false, false, true, false)");
	...
	public void addCounterBatch(Connection con, long lUID, String strQuery, long lCalls, long lDuration_ms) throws Exception {
		...
		try{
			sbQuery.append(strMSSQLSlowProceduresQuery.replaceAll("@KEY@", strPartitionKey))
					.append("(")
					.append(lUID).append(", ")
			...
			stmtMSSQLSlowProcedure.addBatch(sbQuery.toString());
...
//					psDotNetProfiler = hmPreStmtDotNetProfiler.get(strPartitionKey);
					...
					ins = stmtMSSQLSlowProcedure.executeBatch();
					
					// keep track of inserted count
					nInserted += ins.length;
//				}
				
				LogManager.logDBInserts("MSSQL Slow Procedure thread "+lDistributorThreadId+" inserted: "+nInserted+" <> UID: "+strUID);
...
//		iterKeys = null;
	...
	public void execute(Connection con, long lUID, String strQuery, long lCalls, long lDuration_ms) throws Exception {
		...
		try{
			...
			stmtMSSQLSlowProcedure.executeUpdate(sbQuery.toString());
	...
	public void clearPreparedStatement() {
		DataBaseManager.close(stmtMSSQLSlowProcedure);
		stmtMSSQLSlowProcedure = null;
