...
/**
 * PGSlowQueries related Database Interface layer.
 * This do the operations related to the PGSlowQueries records table.
 * 
 * @author Ramkumar
 *
 */
public class MySQLSlowQueryDBI {
	
	private String strMySQLSlowQueriesQry = null;
...
//	private PreparedStatement psPGSlowQueries = null;
	...
	private Statement stmtMySQLSlowQueries = null;
...
//	private String strUID = "";
	...
	public void initializeInsertBatch(Connection con) throws Exception {
		...
		try{
			strMySQLSlowQueriesQry = "INSERT INTO mysql_slowQuery_@KEY@ (uid, appedo_received_on, start_time, Query, calls, duration_ms) VALUES ";
			
			stmtMySQLSlowQueries = con.createStatement();
	...
	public void addCounterBatch(Connection con, long lUID, String strQuery, long lCalls, long lDuration_ms) throws Exception {
		...
		try{
			sbQuery.append(strMySQLSlowQueriesQry.replaceAll("@KEY@", strPartitionKey))
					.append("(")
					.append(lUID).append(", ")
			...
			stmtMySQLSlowQueries.addBatch(sbQuery.toString());
...
//					psPGSlowQueries = hmPreStmtPGSlowQueries.get(strPartitionKey);
					...
					ins = stmtMySQLSlowQueries.executeBatch();
...
//		iterKeys = null;
	 ...
	 * @throws Exception
	 */
	public void execute(Connection con, long lUID, String strQuery, long lCalls, long lDuration_ms,String time) throws Exception {
		StringBuilder sbQuery = new StringBuilder();
		String strPartitionKey;
		...
		try{
			...
			sbQuery.append(strMySQLSlowQueriesQry.replaceAll("@KEY@", strPartitionKey))
					...
					.append("getgmtnow(), ")
					.append("to_timestamp('").append(time)
					.append("','YYYY-MM-DD HH24:MI:SS')::timestamp without time zone,")
					.append("'").append(strQuery).append("'").append(", ")
					.append(lCalls).append(", ")
					.append(lDuration_ms)
					.append(")");
			stmtMySQLSlowQueries.executeUpdate(sbQuery.toString());
			
		} catch(Exception ex) {
			if ( ! ex.getLocalizedMessage().contains("duplicate key value violates unique constraint") ) {
				LogManager.errorLog(ex);
				LogManager.infoLog("Query :"+sbQuery.toString());
				throw ex;
			}
		} finally {
			UtilsFactory.clearCollectionHieracy(sbQuery);
	...
	public void clearPreparedStatement() {
		DataBaseManager.close(stmtMySQLSlowQueries);
		stmtMySQLSlowQueries = null;
